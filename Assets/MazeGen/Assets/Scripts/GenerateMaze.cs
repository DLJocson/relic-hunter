...
public class GenerateMaze : MonoBehaviour
{
    ...
    public void ApplyExitRoomVisual()
    {
        ...
        floorRenderer.color = new Color(0.35f, 1f, 0.45f, 1f);
    ...
    private void PlaceProceduralExit()
    {
        Vector2Int playerStart = Vector2Int.zero;
        Vector2Int guardStart = new Vector2Int(numX - 1, numY - 1);
        int minDistance = (numX + numY) / 2;
        ...
        for (int x = 0; x < numX; x++)
        {
            for (int y = 0; y < numY; y++)
            {
                ...
                if (cell == playerStart || cell == guardStart) continue;
                candidates.Add(cell);
            }
        }
        ...
        {
            ExitCell = guardStart;
            ExitOpeningDirection = Room.Directions.TOP;
        }
        else
        {
            Vector2Int exitCandidate = candidates[0];
            bool found = false;

            for (int attempt = 0; attempt < candidates.Count * 10; attempt++)
            {
                exitCandidate = candidates[UnityEngine.Random.Range(0, candidates.Count)];
                if (ManhattanDistance(exitCandidate, playerStart) >= minDistance)
                {
                    found = true;
                    break;
                }
            ...
            if (!found)
            {
                exitCandidate = candidates[0];
                int bestDistance = ManhattanDistance(exitCandidate, playerStart);

                foreach (Vector2Int cell in candidates)
                {
                    int distance = ManhattanDistance(cell, playerStart);
